...
interface TextinputProps {
  ...
  onFocus?: () => void;
  defaultValue?: string;
  [key: string]: any; // Additional props if any
}
      ...
      <div className={`relative ${horizontal ? "flex-1" : ""}`}>
        ...
        <div className="absolute top-1/2 flex -translate-y-1/2 space-x-1 text-xl ltr:right-[14px]  rtl:left-[14px] rtl:space-x-reverse">
          {hasicon && (
            ...
            >
              {open && type === "password" && (
                <Icons icon="heroicons-outline:eye" />
              )}
              {!open && type === "password" && (
                <Icons icon="heroicons-outline:eye-off" />
              )}
            </span>
          )}
          ...
          {error && (
            <span className="text-danger-500">
              <Icons icon="heroicons-outline:information-circle" />
            </span>
          )}
          {validate && (
            <span className="text-success-500">
              <Icons icon="bi:check-lg" />
            </span>
          )}
        </div>
      </div>
