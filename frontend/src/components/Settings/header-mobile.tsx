...
const HeaderMobile = () => {
  ...
  return (
    ...
    >
      <motion.div
        className="absolute inset-0 right-0 w-full bg-[#FEEBD4] dark:bg-gradient-to-t from-primary to-secondary dark:text-white"
        variants={sidebar}
      />
      <motion.ul
        variants={variants}
...
const MenuToggle = ({ open, toggle }: { open: boolean; toggle: any }) => (
  <button
    onClick={toggle}
    className="pointer-events-auto absolute right-4 top-[14px] z-30 text-2xl text-black dark:text-white"
  >
    {!open ? <CgMenuRightAlt /> : <RxCross2 />}
  </button>
);
